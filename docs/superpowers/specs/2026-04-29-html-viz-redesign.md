...
### Panel: Vault-fitted UMAP — clickable points

- Same Plotly scatter as current columns 2/5 (vault background + transformed RSS entries, kept/rejected coloured).
- Repositioned to columns 2/4 in the new layout.
- Each RSS entry point carries its URL in `customdata`. Vault background points have no URL.
- On `plotly_click`, the HTML fires `window.parent.postMessage({ type: 'rss-viz-click', url, title }, '*')`.
- A `dataviewjs` block in the Obsidian note listens for that message and renders the article title as a clickable link directly in the note, below the iframe. This sidesteps Obsidian's iframe sandbox which blocks `window.open` and `target="_blank"`.
- The Obsidian note template (generated by `note_writer.py`) must include the `dataviewjs` listener block. The iframe `src` path is unchanged.
...
- `rss_filter/score_viz.py` — remove RSS-only UMAP traces and layout axes (x3/y3, x6/y6), replace scatter traces (x1/y1, x4/y4) with per-feed bar traces, update layout domains, add `customdata` (URL per point) to UMAP traces, add `plotly_click` postMessage handler in the emitted JavaScript.
- `rss_filter/note_writer.py` — add the `dataviewjs` postMessage listener block to the generated Obsidian note template, below the iframe. The `threshold_reading` and `threshold_arxiv` parameters remain in `write_score_viz`'s signature for backwards compatibility but are no longer used to draw threshold lines.
